var ComicBubblesEditor = function(cb, cbCanvasWrapper, cbCanvas, cbCanvasContext, canvasWidth, canvasHeight, bubbles, settingsBox, common_fonts, colors140, touchy, tails, tailSize, tailMargin, MINSIZE){
...
this.Settings = function(settings_loaded){
	...
	if (!styleElement) {
		...
		var styles = '.cb-settings { background: #f2f2f2; padding: 4px 3px; margin: 0; border: 1px solid black; -moz-box-sizing: border-box; -webkit-box-sizing: border-box; box-sizing: border-box; }\n';
		styles += '.cb-settings-color-picker { padding: 0 0 0 1px; margin: 3px 0 1px 0; border: 0; width: auto; max-height: 45px; overflow: auto; }\n';
		styles += '.cb-settings-color-picker > div { float: left; width: 16px; height: 15px; padding: 0; margin: 0 1px 1px 0; cursor: pointer; -moz-box-sizing: border-box; -webkit-box-sizing: border-box; box-sizing: border-box; }\n';
		styles += '.cb-settings-color-picker > div[data-bgc="#ffffff"] { clear: left; }\n';
		styles += '.cb-settings-box-button { width: 14px !important; height: 14px !important; background: red !important; position: absolute !important; right: -1px; top: -1px; cursor: pointer; padding: 0 !important; margin: 0 !important; border: 0 !important; font: bold 14px Verdana !important; color: #fff !important; -moz-box-sizing: border-box !important; -webkit-box-sizing: border-box !important; box-sizing: border-box !important; line-height: 13px !important; text-align: center !important; text-transform: none !important; }\n';	
		...
		styles += '.cb-settings-font-family { margin: 2px 0 0 5px; padding: 0; border: 1px solid gray; font: normal normal 13px Arial; height: 22px; width: 149px; }\n';			
		styles += '.cb-settings-font-size { margin: 2px 0 0 3px; padding: 0; border: 1px solid gray; font: normal normal 13px Arial; height: 22px; width: 42px; }\n';
		styles += '.cb-settings-font-weight { margin: 5px 0 0 5px; padding: 0; border: 1px solid gray; font: normal normal 13px Arial; height: 22px; width: 125px; }\n';		
		styles += '.cb-settings-text-align { margin: 0 0 0 3px; padding: 0; border: 1px solid gray; font: normal normal 13px Arial; height: 22px; width: 67px; }\n';		
		styles += '.cb-settings-color-div1, .cb-settings-color-div2, .cb-settings-transparency-div { position: relative; display: block; width: 192px; font: normal normal 14px Arial; color: #0000cd; }\n';
		styles += '.cb-settings-color-div1, .cb-settings-color-div2 { text-align: right; margin: 0 0 0 5px; border: 1px solid #f2f2f2; padding: 4px 2px 4px 2px; }\n';
		styles += '.cb-settings-transparency-div { text-align: right; margin: 0 0 0 5px; border: 0; padding: 5px 0 0 3px; }\n';
		styles += '.cb-settings-color-div1 { border-top: 1px solid gray; padding-top: 5px; padding-bottom: 0; }\n';
		styles += '.cb-settings-bg-color, .cb-settings-font-color { margin: 0; padding: 2px; border: 1px solid gray; font: normal normal 14px Arial; height: 16px; width: 90px; }\n';
		styles += '.cb-settings-transparency { margin: 0; padding: 2px; border: 1px solid gray; font: normal normal 14px Arial; height: 16px; width: 40px; }\n';
		styles += '.cb-settings-color { border: 3px solid red; }\n';
		styles += '.cb-settings-transparency-slider { width: 100%; margin: auto; padding: 3px 0 0 0; }\n';
	...
	settingsBoxBtn.innerHTML = "s";
	settingsBox.appendChild(settingsBoxBtn);
	settingsBoxBtn.onmousedown = function(e){
		e.stopPropagation();
		if (parseInt(settingsBox.style.width) < 100) {
			settingsBox.style.width = '218px';
			settingsBox.style.height = 'auto';
			getElementByClass('cb-settings-box-button').innerHTML = "x";
			closeColorPicker(e);
			if (cb.getBID() > -1) {
			...
			settingsBox.style.width = '14px';
			settingsBox.style.height = 0;
			getElementByClass('cb-settings-box-button').innerHTML = "s";
	...
	var settingsSpan3 = document.createElement('span');
	settingsSpan3.className = "cb-settings-transparency-div";
	settingsSpan3.appendChild(inputInfo3);
	settingsSpan3.appendChild(settingsTransparency);
	settingsSpan3.appendChild(settingsTransparencySlider);	
	settingsSpan3.onmousedown = function(e){
		e.stopPropagation();
	}	
	settingsContainer.appendChild(settingsSpan3);
	settings_loaded();
}
...
function cbCanvasMouseMove(e){
	e.preventDefault();
	var p = getOffset(cbCanvas);
	cb.currentX = e.pageX - p.x;
	cb.currentY = e.pageY - p.y;
	if (cb.getBID() > -1) {
...
function setCoordinates(e){
	var p = getOffset(cbCanvas);
	cb.mouseX = e.pageX - p.x;
	cb.mouseY = e.pageY - p.y;
}

function pressed(e){ 
	var e = window.event || e;
	var inactive = document.activeElement ? (document.activeElement.tagName.toLowerCase() != "p" ? true : false) : true;
	if (e.keyCode == 46 && cb.getBID() > -1 && inactive && parseInt(settingsBox.style.width) < 100) {
		e.preventDefault();
...
this.addSettingsBox = function(sb_x, sb_y){
	sb_y = sb_y + 6;
	settingsBox.style.cssText = 'width: 14px; height: 0; position: absolute; left: ' + sb_x + 'px; top: ' + sb_y + 'px';
	resetZIndex();
